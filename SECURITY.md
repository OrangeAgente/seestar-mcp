...
| **Over-permissioned tools** | 34 single-purpose, least-privilege tools on `seestar-mcp` (plus 5 on the separate `seestar-refine` service) — each does exactly one thing. Motion/destructive tools (`goto_target`, `run_autofocus`, `set_filter`, `set_dew_heater`, `park`, `shutdown`, `download_subs`, `qa_session_report`) are clearly labelled, and the `run-session` / `anomaly-playbook` Skills gate motion and destructive operations behind **explicit user confirmation**. |
